...
if length(ARGS) < 1
    ...
    json_input = String(dirname(@__FILE__)) * "/../input_sample/input_sample_j1j2.json"
else
...
model = "J1-J2"
J1 = 1.0
J2::Float64 = Float64(input_data["J2"])
...
@time begin
    ...
    pprint_diag(spec)
    wpeps, su_hamilt, bond_operators, diag_operators = init_heisenberg(h; J2=J2)
...
for β in beta_values
    ...
    Logging.with_logger(Logging.ConsoleLogger(stdout, Logging.Error)) do
        @time global wpeps, _ = simpleupdate(wpeps, su_hamilt, su_alg; check_interval=2^62)
    end
    ...
    observables = compute_obs(ψ, env, bond_operators, diag_operators; J2=J2)
    println()
